  ...
  <div class="TopIndex">
    <div class="TopIndList" v-for="(item, index) in IndexData" :key="index">
      ...
      <div v-else class="TopITitle">
        {{ item.title }}
        <a-popover v-if="item.remarks===true"  placement="bottomLeft" overlayClassName="OperateDashboard_T_popover">
          <template #content>
  ...
  methods: {
    HandleColor(Cval, Rval, val,Sign) {
      if (this.OnType === 'BShopDSR') {
        ...
        if(Rval === 1) return (val)? (val>0)?'red':'green':''
      }else if(this.OnType === 'AfterSalesExpenses'){
        if(Rval === 0||Rval === 2) return (val)? (val<0)?'red':'green':''

      }
    },
    HandVal(Cval, Rval, val,Sign) {
      if (this.OnType === 'OrderDistributionV2') {
        if (Cval === 0&&Rval === -1) {
          return HandleNum('round', val)
        }else if(Rval === 0||Rval === 1||Rval === 2||Rval === 3){
          return HandleNum('percent1', val)
        }
      }else if(this.OnType === 'OrderDistributionV2_T'){
        // console.log(Cval);
        if(Rval === -1){
          return HandleNum('round', val)
        }else if(Rval === 0){
