<template>
  <div>
    <a-table class="y-table" v-bind="options" v-on="$listeners" :columns="options.columns" size="small"
    :data-source="options.data"
    :bordered="false"
    :pagination="options.needPagingDefault ? options.pagination : false"
    ...
    <y-pagination v-show="options.hasOwnProperty('needPaging') ? options.needPaging : true" :pagination="options.pagination"></y-pagination>
...
export default {
  ...
  methods: {
    initOptions () {
      const _this = this
      const { columns = [], data, pagination } = this.options
