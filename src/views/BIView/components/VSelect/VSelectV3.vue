<template>
    <div class="flex flex-between mr20 text-xs select-container" ref="container"
         style="width: 100%; position: relative;">
        <div class="flex flex-start" style="overflow: hidden" :style="labelWidth">
            <span style="font-size: 14px; color: rgba(0,0,0,0.65)">{{ label }}</span>
        </div>
      ...
      color: rgba(0, 0, 0, .65);
      line-height: 32px; border-radius: 2px; cursor: pointer">
                <div style="flex: 1; overflow: hidden">
                    <span class="nowrap" v-if="query.length">{{ query.join('; ') }}</span>
                </div>
            </div>
            <template v-if="allowShowClear">
            ...
            <a-icon type="close-circle" style="color: #999"></a-icon>
                ...
                <div class="dropdown-button" v-if="!value.length">
                    <a-icon type="double-right"
                            style="color: #079909;transform: rotate(90deg); cursor: pointer;margin-top: 5px;vertical-align: top"/>
                    <!-- <a-icon type="down" class="drop-down-icon"/> -->
                </div>
            </template>
            <template v-else>
                <div class="dropdown-button" style="box-shadow: none;">
                    <!--                <a-icon type="double-right" style="color: #079909;transform: rotate(90deg); cursor: pointer;margin-top: 5px;vertical-align: top"/>-->
                    <a-icon type="down"
                            style="transition: all 0.1s;color: rgba(0, 0, 0, 0.25);cursor: pointer;margin-top: 5px;vertical-align: top"
                            :style="{transform: !tooltipVisible ? 'rotate(0deg)' : 'rotate(180deg)'}"
                    />
                    <!-- <a-icon type="down" class="drop-down-icon"/> -->
                </div>
            </template>
                        ...
                        <div style="font-size: 12px">
                            ...
                            <label class="custom-checkbox-label" title={this.source.label} onClick={(e) => {
                                ...
                                this.emitEvents(this.source)
                                   ...
                                   style="display: block; cursor: pointer; padding: 0 6px;"
