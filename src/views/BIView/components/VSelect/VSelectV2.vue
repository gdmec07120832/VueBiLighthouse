...
export default {
  ...
  watch: {
    value: {
      immediate: true,
      handler (val) {
        this.dataOptions = this.storeDataOptions.concat()
        if (this.multiple) {
          this.query = val
          ...
          let trueArr = []
          this.dataOptions.filter((item,index) => {
            return item.checked === true&&trueArr.push(index)
          })
          trueArr.forEach((i) => {
            let sub = this.dataOptions.splice(i,1)
            this.dataOptions.unshift(sub[0])
          })
          this.filteredOptions = this.dataOptions.concat()
        } else {
          this.query = [val];
    ...
    options: {
      handler (val, oldVal) {
        ...
        this.storeDataOptions = this.dataOptions.slice()
