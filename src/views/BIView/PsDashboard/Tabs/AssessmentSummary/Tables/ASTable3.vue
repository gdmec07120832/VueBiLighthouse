  ...
  <table class="dashboard-common-table">
    ...
    <tr>
      ...
      <td @mouseenter="mouseenter(1, (m+'月'),null)"
        ...
        {{
          formatW(
              ((yearMonthValues[year] || [])[m - 1] || {})['REM_INV_AMT']
          )
        }}
      </td>
    </tr>
    <tr>
      ...
      <td @mouseenter="mouseenter(2, (m+'月'),null)"
        ...
        {{
          formatW(
              ((yearMonthValues[year - 1] || [])[m - 1] || {})['REM_INV_AMT']
          )
        }}
      </td>
    </tr>
...
export default {
  ...
  methods: {
    calcYoy (year, m) {
      const a = this.yearMonthValues[year]?.[m]?.['REM_INV_AMT']
      const b = this.yearMonthValues[year - 1]?.[m]?.['REM_INV_AMT']
      return (a && b) ? (a / b) - 1 : null
    },
    getYearTotal (year) {
      const arr = this.yearMonthValues[year] || []
      return arr.reduce((acc, cur) => {
        return acc + (cur['REM_INV_AMT'] || 0)
