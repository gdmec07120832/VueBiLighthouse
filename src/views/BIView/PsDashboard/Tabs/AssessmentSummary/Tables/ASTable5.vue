  ...
  <table class="dashboard-common-table">
    ...
    <tr>
      ...
      <td @mouseenter="mouseenter(1, (m+'月'),null)"
        ...
        {{
          formatRto(
              ((yearMonthValues[year] || [])[m - 1] || {})['SHBLS_RTO']
          )
        }}
      </td>
    </tr>
    <tr>
      ...
      <td @mouseenter="mouseenter(2, (m+'月'),null)"
        ...
        {{
          formatRto(
              ((yearMonthValues[year - 1] || [])[m - 1] || {})['SHBLS_RTO']
          )
        }}
      </td>
    </tr>
...
export default {
  ...
  methods: {
    calcYoy (year, m) {
      const a = this.yearMonthValues[year]?.[m]?.['SHBLS_RTO']
      const b = this.yearMonthValues[year - 1]?.[m]?.['SHBLS_RTO']
      return (a && b) ? (a / b) - 1 : null
    },
    getYearTotal (year) {
      const arr = this.yearMonthValues[year] || []
      const _arr = arr.slice(0, this.year === year ? this.month + 1 : 12)

      const a = _arr.reduce((acc, cur) => {
        return acc + (cur['SHBLS'] || 0)
      }, 0)

      const b = _arr.reduce((acc, cur) => {
        return acc + (cur['DLVRED_QTY'] || 0)
      }, 0)
