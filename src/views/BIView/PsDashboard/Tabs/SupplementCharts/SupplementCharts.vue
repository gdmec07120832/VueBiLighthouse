  ...
  <div class="px15">
    ...
    <div class="flex-between" >
      <div class="flex flex-start" style="padding: 10px 0px;">
        <!-- <span class="chart-sub-title" style="line-height: 41px;">补件达交</span> -->
        <Title :label="'补件达交'" />
      </div>
    ...
    <div class="flex-between" style="align-items: flex-start">
      <div style="width: calc(60% - 10px)">
        <div class="flex-start">
          <div style="width: 33.33%">
            <div class="text-xs" style="line-height: 30px">当月累计补件达交率</div>
            ...
            <div class="flex-start" style="height: 30px">
              ...
              <span class="text-xs">
                 {{ overviewData.TM_TARGET ? numeral(overviewData.TM_TARGET).format('0.00%') : '--' }}
              </span>
            </div>
            <div class="flex-start" style="height: 30px">
          ...
          <div style="width: 33.33%">
            <div class="text-xs" style="line-height: 30px">年度累计补件达交率</div>
            <div style="font-size: 16px; font-weight: 600; line-height: 40px">
      ...
      <div style="width: calc(40% - 10px)">
        <div class="flex-between" style="height: 30px">
          <div class="text-black">项目组达交率</div>
          <div>
...
export default {
  name: 'SupplementCharts',
  components: { SortTable, EzMonthSelect,Title },
  data () {
  ...
  methods: {
    numeral,
    getData1 () {
      this.$fetchSql('pds_cockpit', 'rfe_deal_tm_spl_cmpl_rto', {
    ...
    getData2 () {
      this.$fetchSql('pds_cockpit', 'rfe_deal_year_spl_cmpl_rto', {
        YDATE: moment(this.query.yearMonth).format('YYYY'),
        MDATE: moment(this.query.yearMonth).format('YYYYMM')
      }).then(({ data }) => {
        this.table.totalRow.data = data[0]
    ...
    getData3 () {
      this.$fetchSql('pds_cockpit', 'rfe_deal_sum_cmpl_rto', {
        YDATE: moment(this.query.yearMonth).format('YYYY'),
        MDATE: moment(this.query.yearMonth).format('YYYYMM')
      }).then(({ data: [ret] }) => {
    ...
    getData4 () {
      this.$fetchSql('pds_cockpit', 'rfe_deal_mly_rto').then(({ data }) => {
        ...
        const calc = (arr) => {
          ...
          const b = arr.reduce((acc, cur) => {
            return acc + (cur['PUR_BILLNO_QTY'] || 0)
          }, 0)
          return b ? a / b : null
        }
