...
export default {
    ...
    computed: {
        SHOP_GROUPAndSHOP_NAME(){
            return this.SHOP_GROUP.toString() + '&&' + this.SHOP_NAME.toString()
        }
    },
    watch: {
        SHOP_GROUPAndSHOP_NAME: {
            handler(){
                this.$emit('changeSelectOptions', {
                    name: '线上店铺',
                    SHOP_GROUP: this.SHOP_GROUP.toString(),
    ...
    methods: {
        ...
        async getData() {
            ...
            // this.$emit('callBack', {data: res.data, name: 'select1'})
