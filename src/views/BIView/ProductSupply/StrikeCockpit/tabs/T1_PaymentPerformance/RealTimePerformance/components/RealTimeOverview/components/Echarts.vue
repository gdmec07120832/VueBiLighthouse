...
<template>
    <div class="echarts">
        <div id="line1" ref="line1" class="left"></div>
        <div id="line2" ref="line2" class="right"></div>
    </div>
</template>
...
export default {
    ...
    methods: {
        formatHour(val) {
            if(val === '00') return '0'
            if(val < 10 && val > 0) return val.split('0')[1]
            else return val
        },
        initLine(id,option) {
            if(this[id] === null) {
        ...
        async getData() {
            ...
            if(res.data.length){
                ...
                let keys = ['TDATE_HOURS', 'SALE_AMT']
                // 图一 第一条线 当前
                this.option1.series[0].data = res.data.map((item, index) => {
                ...
                // 图二 第一条线 当前
                this.option2.series[0].data = res.data.map((item, index) => {
                    if(item.TDATE_HOURS > moment().format('HH')) return 'null' + index
                ...
                for(let i = 0; i < 24; i++) {
                    let content
                    if(i <= moment().format('HH')) content = [i + '', 0]
                    else content = 'null' + i
                    this.option1.series[0].data.push(content)
                    this.option1.series[1].data.push(content)
                    this.option2.series[0].data.push(content)
                    this.option2.series[1].data.push(content)
                }
            }
            if(document.getElementById('line1')){
                echarts.getInstanceByDom(document.getElementById('line1')).clear()
            }
            if(document.getElementById('line2')){
                echarts.getInstanceByDom(document.getElementById('line2')).clear()
            }
            this.initLine('line1',this.option1)
            this.initLine('line2',this.option2)
        },
