<template>
    <div class="StoreDetails">
        ...
        <div class="bottomBox">
            ...
            <div class="right">
<!--                <div>{{table.currentClick.value}}（{{selects[0].value}}）</div>-->
                <div>{{table.currentClick.i == 0 ? '合计' : table.currentClick.value}}（{{selects[0].value}}）</div>
                <AnySizeTitle title="业绩达成趋势"/>
                <v-chart ref="barAndLine1" :options="barAndLine" style="overflow: visible;height: calc(50% - 33.5px);" class="Circle mb10" autoresize></v-chart>
                <AnySizeTitle title="业绩同期对比"/>
                <v-chart ref="barAndLine2" :options="line" style="overflow: visible;height: calc(50% - 33.5px);" class="Circle" autoresize></v-chart>
            </div>
        </div>
    </div>
</template>
...
export default {
    ...
    props: {
        ...
        tableApi: {
            type: String,
            default: 'all_center_pay_shop_offline_dtl',
        },
        trendApi: {
            type: String,
            default: 'all_center_pay_shop_offline_trend',
        },
    },
    async created() {
        let that = this
        ...
        return {
            ...
            table: {
                ...
                tableColumn: [
                    ...
                    {key: 'PTD_PAY_TGT', areaName: 'b', valueFormat: 'round', sumFirstFormat: 'round', sort: null},
                    {key: 'PTD_PAY_AMT', areaName: 'c', valueFormat: 'round', sumFirstFormat: 'round', sort: 'down'},
                    {key: 'PTD_PAY_RATE', areaName: 'd', valueFormat: 'percent', sumFirstFormat: 'percent', colorFormat: 'reach', sort: null},
                    {key: 'AGO_PAY_AMT', areaName: 'e', valueFormat: 'round', sumFirstFormat: 'round', sort: null},
                    {key: 'YOY_PAY_RATE', areaName: 'f', valueFormat: 'percent', sumFirstFormat: 'percent', colorFormat: 'YOY', sort: null},
                ],
                // 合并项
                // 首行或首列 尾行或尾列
                sum: {
    ...
    methods: {
        ...
        handleData(type, source){
            if(type === 'table'){
                ...
                let keys = ['TDATE_DAY', 'PTD_PAY_AMT', 'PTD_PAY_TGT']
                let keys1 = ['TDATE_DAY', 'PTD_PAY_AMT', 'AGO_PAY_AMT']
                this.barAndLine.xAxis[0].data = Array.from(new Set(arr.map(_ => _.TDATE_DAY)))
                this.barAndLine.series[0].data = arr.map(item => {
                    return keys.map(key => {
                        if(key === 'PTD_PAY_AMT' && moment(item.TDATE + '').format('x') > moment().format('x')) return null
                        return item[key]
                    })
                })
