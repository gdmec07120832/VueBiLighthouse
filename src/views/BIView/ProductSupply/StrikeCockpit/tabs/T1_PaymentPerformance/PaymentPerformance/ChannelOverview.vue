    ...
    <div class="ChannelOverview">
        <div class="panelBox" style="cursor: pointer">
            <div class="circlePanel" :class="active('全司')" @click="clickPanel('全司')">
                <CircleEchart v-bind="circle" class="circle"/>
...
export default {
    ...
    props: {
        ...
        panelApi: {
            type: String,
            default: 'all_center_pay_chnl_sum',
        },
        trendApi: {
            type: String,
            default: 'all_center_pay_chnl_trend',
        },
    },
    async created() {
        ...
        return {
            ...
            circlePanel: {
                ...
                map: [
                    ...
                    {areaName: 'b', keyName: 'PTD_PAY_AMT', className: 'subTitle', valueFormat: 'round'},
                    {areaName: 'c', value: '目标', className: 'label'},
                    {areaName: 'd', keyName: 'PTD_PAY_TGT', className: 'value',  valueFormat: 'hundredMillion_double_dot'},
                    {areaName: 'e', value: '同比', className: 'label'},
                    {areaName: 'f', keyName: 'YOY_PAY_RATE', className: 'value', valueFormat: 'percent'},
                    {areaName: 'g', value: '差额', className: 'label'},
                    {areaName: 'h', keyName: 'PTD_PAY_DIFF', className: 'value', valueFormat: 'hundredMillion_double_dot', colorFormat: 'diff'},
                    {areaName: 'i', value: '同期', className: 'label'},
                    {areaName: 'j', keyName: 'AGO_PAY_AMT', className: 'value', valueFormat: 'hundredMillion_double_dot'},
                ],
                dataSource: {}
            },
            currentPanel: '全司',
            config: [
                {
                    ...
                    map: [
                        ...
                        {areaName: 'h', keyName: 'PTD_PAY_RATE', className: 'value', valueFormat: 'percent', colorFormat: 'reach'},
                        {areaName: 'i', value: '占总', className: 'label'},
                        {areaName: 'j', keyName: 'TOT_PAY_RATE', className: 'value', valueFormat: 'percent'},
                    ],
                    dataSource: {}
                },
                {
    ...
    methods: {
        ...
        handleData(type, source){
            if(type === 'panel'){
                ...
                this.circle.value = source.filter(_ => _.CHNL_NAME === '全司')[0].PTD_PAY_RATE
                ...
                let keys = ['TDATE', 'PTD_PAY_AMT', 'PTD_PAY_TGT']
                ...
                let keys = ['TDATE', 'PTD_PAY_TGT', 'PTD_PAY_AMT', 'PTD_PAY_RATE', 'YOY_PAY_RATE']
                let arr = _.cloneDeep(source)
                arr.sort((a, b) => a.TDATE - b.TDATE)
                ...
                let setData = function(tableName, data) {
                    that[tableName].tableData = data.map(item => {
                        return keys.map(key => {
                            ...
                            if(['PTD_PAY_AMT', 'PTD_PAY_RATE', 'YOY_PAY_RATE'].includes(key) && moment(item.TDATE + '').format('x') > moment().format('x')) return null
                            return item[key]
                        })
                    })
                }
