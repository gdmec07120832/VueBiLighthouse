    ...
    <div class="tablelthead" style="padding-right: 8px;">
      <table class="TRORCLeftTUser2">
        <thead>
          ...
          <tr v-if="IsRankOn">
            <div v-for="(item, index) in labelD" :key="index" class="TitleHand IsRank">
              <td v-if="item!=='-1'" :title="item.replace(/[RANK]/g,'')" :class="[(index===0)?'Title':'Con'+index,(item.indexOf('RANK')>0)?'OnR':'']"
                @click="(item.indexOf('RANK')>0)?OnIsRankFun(index):''"
                >
                <div class="IsRankAllCss">
                  <div class="THName">{{ item.replace(/[RANK]/g,'') }}</div>
                  <Sort v-if="(item.indexOf('RANK')>0)" 
                    :current='(OnNum===index)?IsCurrent:null' 
  ...
  methods: {
    ...
    HandleColor(Cval, Rval, val) {
      if (this.index === 'T2_Activity') {
        if(Rval===0) return ''
          else if((Cval===2||Cval===4)&&Rval!==0) return  (val>1)?'red':'green'
          else return ''
      }else if(this.index === 'StoreBPreSale'){
        if(Rval===5) return (val>1)?'red':'green'
          else return ''
      }else if(this.index === 'KAReach'){
        if(Rval===0) return ''
          else if(Cval===4) return (val)?(val>1)?'red':'green':''
          else if(Cval===3) return (val)?(val>0)?'red':'green':''
      }else if(this.index === 'GrossProfitA'||this.index === 'GrossProfitB'){
        // console.log("=====")
        if(this.index === 'GrossProfitA')
          if(Rval===4||Rval===8) return (val>1)?'red':'green'
            // else if(Cval === 'Total'&&(Rval===4||Rval===8)) return (val>1)?'red':'green'
            else return ''
        else if(this.index === 'GrossProfitB')
          if(Rval===5||Rval===9) return (val>1)?'red':'green'
            else return ''
      }else if(this.index === 'UnsalableAmountA'){
        if(Rval===2||Rval===4) return (val)? (val<0)? 'red':'green':''
      }else if(this.index === 'GuestListA'){
        if(Rval===0) return ''
          else if(Rval===3) return (val)? (val>0)?'red':'green':''
      }else if(this.index === 'GuestList_2A'){
        if(Rval===0||Rval===1) return ''
          else if(Rval===4) return (val)? (val>0)?'red':'green':''
      }else if(this.index === 'GuestListB'){
        if(Rval===0) return ''
        else if(Cval===3) return (val)? (val>0)?'red':'green':''
      }else if(this.index === 'CityYOYNum'){
        if(Rval===0) return ''
          else if(Rval===3) return (val)?(val>=0.85)?'red':'green':''
      }else if(this.index === 'CityYOYAmt'){
        return ''
      }
    },
    HandVal(Cval, Rval, val){
      ...
      if(this.index==='T2_Activity'){
        if(Rval===0) return val
          else if((Cval===2||Cval===4)&&Rval!==0) return  HandleNum('percent1', val)
          else return HandleNum('tenThousand2',val)
      }else if(this.index === 'StoreBPreSale'){
        if(Rval===0) return val
          else if(Rval===5||Rval===6||Rval===9) return HandleNum('percent',val)
          else return HandleNum('round',val)
      }else if(this.index === 'KAReach'){
        if(Rval===0) return val
          else if(Cval===0||Cval===1||Cval===2) return HandleNum('HundredMillion',val)
          else if(Cval===3||Cval===4) return HandleNum('percent1',val)
      }else if(this.index === 'GrossProfitA'||this.index === 'GrossProfitB'){
        if(this.index === 'GrossProfitA')
          if(Rval===0||Rval===1) return val
            else if(Rval===2||Rval===3||Rval===5) return HandleNum('tenThousand2',val)
            else if(Rval===4||Rval===6||Rval===7||Rval===8||Rval===9) return HandleNum('percent1',val)
              
        else if(this.index === 'GrossProfitB')
          console.log("================")
          if(Rval===0||Rval===1||Rval===2) return val
            else if(Rval===3||Rval===4||Rval===6) return HandleNum('tenThousand2',val)
            else if(Rval===5||Rval===7||Rval===8||Rval===9||Rval===10) return HandleNum('percent1',val)

      }else if(this.index === 'UnsalableAmountA'){
        if(Rval===0) return val
          else if(Rval===1) return HandleNum('tenThousand',val)
          else if(Rval===2||Rval===3||Rval===4) return HandleNum('percent',val)
      }else if(this.index === 'UnsalableAmountB'){
        if(Rval===0) return val
          else return HandleNum('round',val)
      }else if(this.index === 'GuestListA'){
        if(Rval===0) return val
          else if(Rval===1||Rval===2) return HandleNum('round',val)
          else if(Rval===3) return HandleNum('percent1',val)
      }else if(this.index === 'GuestList_2A'){
        if(Rval===0||Rval===1) return val
          else if(Rval===2||Rval===3) return HandleNum('round',val)
          else if(Rval===4) return HandleNum('percent1',val)
      }else if(this.index === 'GuestListB'){
        if(Rval===0) return val
          else if(Cval===0||Cval===1||Cval===2) return HandleNum('round',val)
          else if(Cval===3) return HandleNum('percent1',val)
      }else if(this.index === 'CityYOYNum'){
        if(Rval===0) return val
          else if(Rval===1||Rval===2) return HandleNum('round',val)
          else if(Rval===3||Rval===4||Rval===5) return HandleNum('percent1',val)
      }else if(this.index === 'CityYOYAmt'){
        if(Rval===0) return val
          else if(Rval===1||Rval===2||Rval===3) return HandleNum('round',val)
          else if(Rval===4||Rval===5) return HandleNum('percent1',val)
      }else{
        return val
      }
