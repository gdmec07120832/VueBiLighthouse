    ...
    <div>
      <div style="background-color: #f5faff; height: 100vh; padding: 0 2%;overflow: auto;">
        <div class="py10" style="margin: auto;">
          <div style="font-size: 26px">货期报表</div>
          <div style="line-height: 24px" class="sub-tip">备注：数据更新周期：每天2点、12点30分、19点30分运行；</div>
        </div>
          ...
          <div class="flex flex-start" style="padding: 20px 20px 0;" >
            <div style="width: 24%; margin-right: 2%">
              <virtual-select v-model="query.TEAM_SUPPLY"
                              ...
                              label="项目组"
                              box-width="223px"
                              :options="teamSupplyOptions.map(_ => ({label: _}))"></virtual-select>
            </div>
...
export default {
    ...
    return{
      ...
      table: {
        ...
        columns: [
           ...
           {title: '客审未货审订单', dataIndex: 'UNGAUD_ORDERS_QTY', width: table_width + 20, ellipsis: true, align: 'right', sorter: true},
  ...
  methods:{
    ...
    getData2(isResetPage = false) {
      ...
      this.$fetchSql('pds_sop', 'bord_valid', params).then(({data: {list, totalRows}}) => {
