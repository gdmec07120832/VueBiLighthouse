 ...
 * @LastEditTime: 2022-02-17 14:50:57
 * @LastEditors: Please set LastEditors
 * @Descripttion: 
 * @FilePath: \lsmy-bi-lighthouse-front\src\views\BIView\ProductSupply\OverseasCockpit\tabs\Comp10\components\Month.vue
-->
<template>
    <div class="Day">
        <div class="header">
        ...
        <div class="footer">
            ...
            <div class="right">
                ...
                :rowKey="(row, index) => row.STAFF_NAME + index"
    ...
    computed: {
        tableColumns() {
            return [
                ...
                { title: '退款金额占比', dataIndex: 'MON_REFUND_AMT_RATE', width: 100, ellipsis: true, sorter: false,
                    customRender: (text) => {
                        return <div oncontextmenu={(e) => {this.openMenu(e,text)}} title={text}>{this.handleNum(2, text)}</div>
                    }
                },
                ...
                { title: '退款数量占比', dataIndex: 'MON_REFUND_CNT_RATE', width: 100, ellipsis: true, sorter: false,
                    customRender: (text) => {
                        return <div oncontextmenu={(e) => {this.openMenu(e,text)}} title={text}>{this.handleNum(2, text)}</div>
                    }
                },
        ...
        return {
            ...
            echartModel1: {
                tooltip: {
                    ...
                    formatter: function(params) {
                        let str = `统计年月：${params[0].data[0]}<br/>退款率：${that.handleNum(2, params[0].data[1])}`
                        return str
                    },
                    backgroundColor: '#fff',
                    textStyle: {
            ...
            echartModel3: {
                ...
                tooltip: {
                    ...
                    formatter: function(params) {
                        let str = `统计年月：${params.data[0]}<br/>人员：${params.seriesName}<br/>退款${that.radioModel === 1 ? '金额' : '数量'}：${that.handleNum(1, params.data[2])}<br/>退款率：${that.handleNum(2, params.data[1])}`
                        return str
                    },
                    backgroundColor: '#fff',
                    textStyle: {
    ...
    methods: {
        ...
        async handleData(type, source) {
            ...
            // 明细
                ...
                //     return source.filter(_ => _.STAFF_NAME === name)
                // })
                let arr = source.filter(_ => people.indexOf(_.STAFF_NAME) > -1)
                console.log(arr);
                // .map(_ => {
                //     return {
                //         STAFF_NAME: _[0].STAFF_NAME,
                //         // 退款金额
                //         REFUND_AMT: _.map(item => item.REFUND_AMT).reduce((a, b) => a + b),
                //         REFUND_CNT: _.map(item => item.REFUND_CNT).reduce((a, b) => a + b)
                //     }
                // })
                ...
                let query = {
                    ...
                    END_TIME: this.dateV2.month2 + '31',
                }
                ...
                // line
                let keys = this.radioModel === 1 ? ['MDATE', 'REFUND_AMT_RATE'] : ['MDATE', 'REFUND_CNT_RATE']
                ...
                // let keys = [this.radioModel === 1 ? 'REFUND_AMT' : 'REFUND_CNT', 'STAFF_NAME', this.radioModel === 1 ? 'MON_REFUND_AMT' : 'MON_REFUND_CNT']
                let keys = [this.radioModel === 1 ? 'REFUND_AMT' : 'REFUND_CNT', 'STAFF_NAME', this.radioModel === 1 ? 'MON_REFUND_AMT_RATE' : 'MON_REFUND_CNT_RATE']
                ...
                let arr = source.concat()
                // x轴
                this.echart3.xAxis.data = Object.freeze(Array.from(new Set(arr.map(_ => moment(_.MDATE).format('YYYY-MM')))))
                // line
                let people = !this.people.length ? this.peopleArr : this.people
                    ...
                    return {
                        ...
                        data: arr.filter(_ => name.indexOf(_.STAFF_NAME) > -1).map(item => {
                            return [
                                ...
                                // moment(item.MDATE).format('YYYY-MM-DD'),
        ...
        // 人员数据 - 右下角echart图 table表
        async getPeople() {
            let query = {
                START_TIME: this.dateV2.month1 + '01',
                END_TIME: this.dateV2.month2 + '31',
            }
            let res =  await this.$fetchSql('oversea_cockpit', 'oversea_goods_refund_stf_m', query)
        ...
        async getPeople1() {
            ...
            let res =  await this.$fetchSql('oversea_cockpit', 'oversea_goods_refund_stf_sm', query)
            // res.data.sort((a, b) => moment(a.MDATE).format('x') - moment(b.MDATE).format('x'))
            console.log(res);
            this.echart2Data = res.data.concat()
            this.handleData(5, res.data)
