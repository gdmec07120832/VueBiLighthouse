 ...
 * @LastEditTime: 2022-02-28 17:10:06
 * @LastEditors: Please set LastEditors
 * @Descripttion: 未发取消
 * @FilePath: \lsmy-bi-lighthouse-front\src\views\BIView\ProductSupply\OverseasCockpit\tabs\Comp15\components\DayV2.vue
-->
<template>
    <div class="Day">
        <div class="header">
        ...
        <div class="middle">
            <div style="margin: 0px 0 0px 0">未发取消率趋势</div>
    ...
    data() {
        ...
        return {
            echartModel1: {
                tooltip: {
                    ...
                    formatter: function(params) {
                        let str = `统计日期：${params[0].data[2]}<br/>未发取消率：${that.handleNum(2, params[0].data[1], true)}`
                        return str
                    },
                    backgroundColor: '#fff',
                    textStyle: {
            ...
            table: {
                type: 2,
                index: '14A',
                labelData: ['统计日期', '支付业绩', '本期未发取消金额', '未发取消率'],
                tableData: []
            },
            table1: {
                type: 2,
    ...
    methods: {
        ...
        async handleData(type, source) {
            ...
            if(type === 1) {
                ...
                // 退款
                let keys = ['REFUND_AMT_RATE', 'REFUND_MONTH_TARGET', 'REFUND_AMT_DIFF', 'REFUND_AMT']
                // 退货
                let keys1 = ['REFUND_CNT_RATE', 'REFUND_MONTH_TARGET', 'REFUND_CNT_DIFF', 'REFUND_CNT']
            ...
            // 明细
                ...
                this.table.labelData =  ['统计日期', '支付业绩', this.radioModel === 1 ? '本期未发取消金额' : '本期未发取消数量', '未发取消率']
                this.table1.labelData = ['统计日期', '支付业绩', this.radioModel === 1 ? '本期未发取消金额' : '本期未发取消数量', '未发取消率']
                if(!source.length) {
