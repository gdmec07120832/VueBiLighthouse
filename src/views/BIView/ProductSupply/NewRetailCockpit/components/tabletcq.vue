        ...
        if(val.value){
          if(this.index==='T3_AttractInvestmentCate'){
            if(Rval===0) return val.value
              else if((Rval===1)) return  this.handleNum('round', val.value)
              else return (val!==0||val)?this.handleNum('percent',val.value):'--'
          }else if(this.index==='T4_BDPen'){
            if(Rval===0||Rval===1) return val
              else return ((val.value!==0||val.value)&&(val.value2!==0||val.value2))?this.handleNum('percent',val.value):'--'
          }
        }else{
          if(this.index==='T3_AttractInvestmentCate'){
            if(Rval===0) return val
              else if((Rval===1)) return  this.handleNum('round', val)
              else return (val!==0||val)?this.handleNum('percent',val):'--'
          }else if(this.index==='T3_AttractInvestmentHard'){
            if(Rval===0) return val
              else if (Rval===3) return (val!==0||val)?this.handleNum('percent',val):'--'
              else return this.handleNum('round', val)
          }else if(this.index==='T4_BDPen'){
            if(Rval===0||Rval===1) return val
              else return (val!==0||val)?this.handleNum('percent',val):'--'
          }
